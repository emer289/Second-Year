...
struct hashtable * hashtable_new(int size)
{
    ...
    assert(result -> table != NULL); //check that memory has been alocated
...
{
      if(item != NULL){
          int i = hash_string(item, this -> size);
          listset_add((this -> table+i), item); // adding the item to the begining of the listset
  }
}
...
{
      int i = hash_string(item, this -> size);
      return listset_lookup((this -> table+i), item);
}
...
{
      int i = hash_string(item, this -> size);
      return listset_remove((this -> table+i), item);
}
